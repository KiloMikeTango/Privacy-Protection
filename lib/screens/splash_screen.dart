...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> _checkSetup() async {
    ...
    try {
      final List<dynamic> result = await _channel.invokeMethod('getSecretPattern');
      ...
      if (current.length < 6) {
        if (mounted) {
          ...
          Navigator.of(context).pushReplacementNamed('/secret_setup', arguments: {'forceSetup': true});
        }
      } else {
        if (mounted) {
          // All good, go home
